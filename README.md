...
El instalador copia el plugin a una carpeta temporal, ajusta la ruta absoluta del helper al `$HOME` del usuario actual (Ryoku exige que sea idéntica en `manifest.json` y en `service/Main.qml`), lo valida con `ryoku plugin validate` y lo instala con `ryoku plugin add --bar --yes`. No modifica el repositorio. Para actualizar una instalación existente usa `./bin/install.sh --reinstall`.
